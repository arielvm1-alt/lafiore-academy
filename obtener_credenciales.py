# -*- coding: utf-8 -*-
"""
Convierte el token corto del Explorador en el de 60 dias y averigua el IG_USER_ID.
...
def _get(ruta, **params):
    ...
    req.add_header("User-Agent", "lafiore-academy-setup/1.0")
    try:
...
def main():
    print("Configuracion de credenciales de La Fiore Academy")
    print("App: %s   API: %s\n" % (APP_ID, VERSION))
    ...
    elegida = None
    for p in vinculadas:
        if "academy" in p["name"].lower():
            elegida = p
            break
    if elegida is None and len(vinculadas) == 1:
        elegida = vinculadas[0]
    if elegida is None:
        print("\nHay varias paginas con Instagram. Elige una:")
    ...
    print()
    print("Cargalos en:")
    print("  https://github.com/arielvm1-alt/lafiore-academy/settings/secrets/actions/new")
    print()
